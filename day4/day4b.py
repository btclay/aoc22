...
for pair in elves:
	...
	elif int(first_range[0]) <= int(second_range[1]) and int(first_range[1]) >= int(second_range[0]):
		total_eclipse_of_the_tasks +=1

	elif int(second_range[0]) <= int(first_range[1]) and int(second_range[1]) >= int(first_range[0]):
		total_eclipse_of_the_tasks +=1
